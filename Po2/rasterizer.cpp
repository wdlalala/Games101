...
static bool insideTriangle(int x, int y, const Vector3f* _v)
{
    ...
    Eigen::Vector3f v0 = _v[1] - _v[0];
    Eigen::Vector3f v1 = _v[2] - _v[1];
    Eigen::Vector3f v2 = _v[0] - _v[2];

    Eigen::Vector3f c0 = p - _v[0];
    Eigen::Vector3f c1 = p - _v[1];
    Eigen::Vector3f c2 = p - _v[2];

    if (v0.cross(c0).z() > 0 && v1.cross(c1).z() > 0 && v2.cross(c2).z() > 0)
        return true;
    if (v0.cross(c0).z() < 0 && v1.cross(c1).z() < 0 && v2.cross(c2).z() < 0)
        return true;
...
rst::rasterizer::rasterizer(int w, int h, int ss) : width(w), height(h), ssaa(ss)
{
    frame_buf.resize(w * h);
    ssaa_buf.resize(w * h * ss * ss);
    depth_buf.resize(w * h * ss * ss);
}
