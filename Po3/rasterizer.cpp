...
void rst::rasterizer::draw(std::vector<Triangle *> &TriangleList) {
    ...
    // 将三角形列表分成多个子列表
    int chunk_size = TriangleList.size() / num_threads;

    auto rasterize_chunk = [&](int start, int end) {
    ...
    for (int i = 0; i < num_threads; ++i) {
        ...
        int end = (i == num_threads - 1) ? TriangleList.size() : (i + 1) * chunk_size;
        threads.emplace_back(rasterize_chunk, start, end);
    }
...
void rst::rasterizer::rasterize_triangle(const Triangle& t, const std::array<Eigen::Vector3f, 3>& view_pos) 
{
    ...
    float invSuperSampleFactor = 1.0f / superSampleFactor;
    ...
    y_min = std::max(y_min, 0.f);
    x_max = std::min(x_max, static_cast<float>(width));
    y_max = std::min(y_max, static_cast<float>(height));
    ...
    for (int x = std::floor(x_min); x <= std::ceil(x_max); x++)
    {
        for (int y = std::floor(y_min); y <= std::ceil(y_max); y++)
        {
            ...
            for (int i = 0; i < superSampleFactor; i++)
            {
                for (int j = 0; j < superSampleFactor; j++)
                {
                    float sub_x = x + (i + 0.5f) * invSuperSampleFactor;
                    float sub_y = y + (j + 0.5f) * invSuperSampleFactor;

                    if (insideTriangle(sub_x, sub_y, t.v))
                    {
                        float alpha, beta, gamma;
                        std::tie(alpha, beta, gamma) = computeBarycentric2D(sub_x, sub_y, t.v);
                        ...
                        if (z_interpolated < min_depth)
                        {
                            ...
                            auto interpolated_color = interpolate(alpha, beta, gamma, t.color[0], t.color[1], t.color[2], w_reciprocal).normalized();
            ...
            final_color /= (superSampleFactor * superSampleFactor);
