                                ...
                                const std::array<int, 3>& dirIsNeg) const
{
	...
	//float tmaxx = (dirIsNeg[0] ? pMax.x : pMin.x - ray.origin.x) * invDir.x;

	//float tminy = (dirIsNeg[1] ? pMin.y : pMax.y - ray.origin.y) * invDir.y;
	//float tmaxy = (dirIsNeg[1] ? pMax.y : pMin.y - ray.origin.y) * invDir.y;

	//float tminz = (dirIsNeg[2] ? pMin.z : pMax.z - ray.origin.z) * invDir.z;
	//float tmaxz = (dirIsNeg[2] ? pMax.z : pMin.z - ray.origin.z) * invDir.z;

	//float tmin = fmax(fmax(tminx, tminy), tminz);
	//float tmax = fmin(fmin(tmaxx, tmaxy), tmaxz);

	//return tmin <= tmax && tmax > 0;

    float tMin = (pMin.x - ray.origin.x) * invDir.x;
    float tMax = (pMax.x - ray.origin.x) * invDir.x;
    if (dirIsNeg[0] == 0) std::swap(tMin, tMax);
