...
Eigen::Matrix4f get_projection_matrix(float eye_fov, float aspect_ratio, float zNear, float zFar)
{
    ...
    projection << 2 * zNear / (right - left), 0, (right + left) / (right - left), 0,
...
Eigen::Vector3f texture_fragment_shader(const fragment_shader_payload& payload)
{
    ...
    for (auto& light : lights)
    {
        ...
        // 计算漫反射光
        auto diffuse = kd.cwiseProduct(I) * std::max(0.0f, normal.dot(light_dir));
        // 计算视线方向向量
        auto view_dir = (eye_pos - point).normalized();
        // 计算半程向量
        auto h_vector = (light_dir + view_dir).normalized();
        // 计算镜面反射光
        auto specular = ks.cwiseProduct(I) * std::pow(std::max(0.0f, normal.dot(h_vector)), p);
        // 累加结果颜色
...
Eigen::Vector3f phong_fragment_shader(const fragment_shader_payload& payload)
{
    ...
    for (auto& light : lights)
    {
        ...
        auto light_dir = (light.position - point).normalized();
        // 计算漫反射光
        auto diffuse = kd.cwiseProduct(light.intensity) * std::max(0.0f, normal.dot(light_dir));
        // 计算视线方向向量
        auto view_dir = (eye_pos - point).normalized();
        // 计算半程向量
        auto h_vector = (light_dir + view_dir).normalized();
        // 计算镜面反射光
        auto specular = ks.cwiseProduct(light.intensity) * std::pow(std::max(0.0f, normal.dot(h_vector)), p);
        // 累加结果颜色
        result_color += ambient + diffuse + specular;
    }
...
Eigen::Vector3f displacement_fragment_shader(const fragment_shader_payload& payload)
{
	...
	auto result_normal = (TBN * ln).normalized();
...
int main(int argc, const char** argv)
{
    ...
    std::string filename = "./Po3/output.png";
    objl::Loader Loader;
    std::string obj_path = "./Po3/models/spot/";
